...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  >
    ...
    <a
      href="#"
      className="text-[#0bb8f9] font-bold hover:underline flex items-center"
    >
      {detailText} <ArrowRight size={16} className="ml-1" />
    </a>
  </motion.div>
);
...
const Home: React.FC = () => {
  ...
  const services: ServiceCardProps[] = [
    ...
    },
    {
      ...
      detailText: '자세히 알아보기',
      icon: <HomeIcon size={24} className="text-[#0bb8f9]" />,
    },
    {
      title: '입주청소',
      description:
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { text: '지금 예약하기', path: '/login' },
            { text: '서비스 알아보기', path: '/service' },
            { text: '사업자등록 알아보기', path: '/ptrecruitment' },
          ].map((item, index) => (
            <motion.button
              key={index}
              onClick={() => navigate(item.path)}
              className="bg-[#0bb8f9] text-white py-3 px-6 rounded-md hover:bg-[#0aa8e9] transition duration-300 flex items-center justify-center"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              {item.text} <ArrowRight size={20} className="ml-2" />
            </motion.button>
          ))}
        </div>
      </motion.section>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            {
              text: '빠르고 간편한 청소 예약',
              description: '클릭 몇 번으로 청소 전문가와 연결됩니다.',
            },
            {
              text: '전문가의 손길',
              description: '청소 전문가가 당신의 집을 완벽하게 관리합니다.',
            },
            {
              text: '시간 절약',
              description: '청소는 깔끔한 방이, 당신은 소중한 일에 집중하세요.',
            },
          ].map((item, index) => (
            <motion.div
              key={index}
              ...
              whileHover={{ y: -5 }}
            >
              <h3 className="text-xl font-bold mb-2 text-[#0bb8f9]">
                {item.text}
              </h3>
      ...
      >
        ...
        <motion.button
          ...
          className="bg-[#0bb8f9] text-white py-3 px-8 rounded-md text-xl hover:bg-[#0aa8e9] transition duration-300 flex items-center justify-center mx-auto"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
        >
          지금 예약하기 <ArrowRight size={24} className="ml-2" />
        </motion.button>
      </motion.section>
