...
import CommissionEstimate from '../pages/partners/CommissionEstimate';

const ProtectedRoute: React.FC<{ allowedRole: 'member' | 'partner' }> = ({ allowedRole }) => {
  const { isAuthenticated, loading, member, partner } = useAuth();
  if (loading) return <div>Loading...</div>;
  if (!isAuthenticated) return <Navigate to="/loginselect" />;
  if (allowedRole === 'member' && !member) return <Navigate to="/loginselect" />;
  if (allowedRole === 'partner' && !partner) return <Navigate to="/loginselect" />;
  return <Outlet />;
};
